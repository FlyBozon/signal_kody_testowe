 ...
 * Stałe
 * ===========================================================================*/
#define MEMS_SIGNAL_SIZE     (uint32_t)(NEAI_INPUT_SIGNAL_LENGTH * NEAI_INPUT_AXIS_NUMBER)
...
int main(void)
{
  ...
  if (neai_err != NEAI_OK) { Error_Handler(); }
  ...
  while (1)
  {
    ...
    {
      case LOG:       Log();                         break;
      case LEARN:     Learn(); appState = INFERENCE; break;
      case INFERENCE: Inference();                   break;
      default:        appState = LOG;                break;
    }
  }
}
...
void MEMS_Init(void)
{
  ...
  IKS4A1_MOTION_SENSOR_DRDY_Enable_Interrupt(IKS4A1_LSM6DSV16X_0,
      MOTION_ACCELERO | MOTION_GYRO, IKS4A1_MOTION_SENSOR_INT1_PIN);
...
void ToF_Init(void)
{
  ...
  if (status != BSP_ERROR_NONE) Error_Handler();
}
...
void PrintToFLine(void)
{
  ...
  if (status != BSP_ERROR_NONE) return;

  tofResultCnt++;
  if (tofResultCnt <= TOF_SKIP_RESULTS) return;  /* poczekaj na stabilne dane */

  printf("TDIST");
  for (i = 0; i < TOF_RESOLUTION; i++)
  {
    int d = (int)Result.ZoneResult[i].Distance[0];
    if (d < 0 || d > (int)DISTANCE_MAX) d = (int)DISTANCE_MAX;
    printf(" %d", d);
  }
  printf("\r\n");
...
void Log(void)
{
  ...
  if (memsDataReady != RESET)
  {
    ...
    printf("MEMS Ax=%d Ay=%d Az=%d Gx=%d Gy=%d Gz=%d\r\n",
           ...
           (int)(gyr.x / 1000), (int)(gyr.y / 1000), (int)(gyr.z / 1000));
...
void Learn(void)
{
  ...
  while (iter < LEARNING_ITERATIONS)
  {
    ...
    printf("LEARN %lu/%lu st=%d\r\n",
           (unsigned long)(iter + 1), (unsigned long)LEARNING_ITERATIONS, (int)st);
    iter++;
  }
  printf("LEARN done\r\n");
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  switch (GPIO_Pin)
  {
    case GPIO_PIN_4:   memsDataReady = SET; break;  /* PB4: MEMS INT1 */
